...
public class PaymentService
{
    ...
    public async Task<IResult> EnqueuePaymentAsync(HttpContext context)
    {
        ...
        await MongoDb.EnqueueWorkAsync(Encoding.UTF8.GetString(rawBody)).ConfigureAwait(false);
    ...
    {
        await MongoDb.PurgePaymentsAsync().ConfigureAwait(false);
        return Results.Ok("Payments removed from MongoDB.");
    }
    ...
    public async Task ProcessPaymentAsync(string message)
    {
        ...
        await MongoDb.EnqueueWorkAsync(message).ConfigureAwait(false);
    }   
}
