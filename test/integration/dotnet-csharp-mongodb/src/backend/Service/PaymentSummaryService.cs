...
public class PaymentSummaryService
{
    ...
    public PaymentSummaryService(
        ...
        MongoDbIntegrationStore mongoDb,
    ...
    public async Task<IResult> GetPaymentsSummaryAsync(DateTimeOffset? from, DateTimeOffset? to)
    {
        ...
        var mongoDbChannelBlockingGate = LockFactory.GetTrackerState(Constant.REACTIVELOCK_MONGODB_NAME);
        ...
        try
        {
            ...
            {
                await mongoDbChannelBlockingGate.WaitIfBlockedAsync().ConfigureAwait(false);
                await channelBlockingGate.WaitIfBlockedAsync().ConfigureAwait(false);
            }, timeout: TimeSpan.FromSeconds(40)).ConfigureAwait(false);

            var payments = await MongoDb.GetPaymentsAsync(from, to).ConfigureAwait(false);
    ...
    {
        ConsoleWriterService.WriteLine("[MongoDB] Gate blocked.");
        ...
        await state.WaitIfBlockedAsync(
            ...
            whileBlockedAsync: async () =>
            {
                try
                {
                    ...
                    if (processedCount > 0)
                    {
                        ConsoleWriterService.WriteLine($"[MongoDB] Processed batch with {processedCount} records.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[MongoDB][Error] Exception while processing message: {ex}");
                }
